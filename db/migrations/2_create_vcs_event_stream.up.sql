CREATE TABLE IF NOT EXISTS vcs_stream(
   id UUID PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS aggregate_id_idx ON vcs_stream (aggregate_id);
CREATE INDEX IF NOT EXISTS  occurred_at_idx ON vcs_stream (occurred_at);
CREATE INDEX IF NOT EXISTS  received_at_idx ON vcs_stream (received_at);
CREATE INDEX IF NOT EXISTS  type_idx ON vcs_stream (type);

-- aggregate id first as i expect it has higher cardinality than type
CREATE INDEX IF NOT EXISTS  type_idx ON vcs_stream (aggregate_id, type);